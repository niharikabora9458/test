(function (w, d, t) {
  ...
  fn.FaqSeeAll = function () {
    ...
    for (temp.i = 0; temp.i < DOM.FaqPara.length; temp.i++) {
      temp.HTML +=
        "<li class='FaqLi' data-index='" +
        temp.i +
        "' >" +
        DOM.FaqPara[temp.i].getAttribute("data-q") +
        "</li>";
    }
    temp.HTML += "</ul>";
  ...
  fn.FaqSlide = function (i) {
    ...
    DOM.FaqQues.innerHTML =
      "<div class='FaqQuesOuter' >" +
      "<div class='FaqBack'>❮ </div><div class='FaqQuesBody' >" +
      temp.DATA +
      "</div></div>";
  ...
  DOM.FaqQues.addEventListener(
    "click",
    function (e) {
      if (
        e.target.className.split(" ").indexOf("FaqQuesOuter") > -1 ||
        e.target.className.split(" ").indexOf("FaqBack") > -1 ||
        e.target.className.split(" ").indexOf("FaqQuesBody") > -1
      ) {
